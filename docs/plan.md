...
| 14:30 | **Code-read** | • [mini-swe-agent](https://github.com/SWE-agent/mini-swe-agent) — an agent of about 100 lines. List what it omits compared with learn-claude-code.<br>• claude-code-from-source [`ch01` architecture](https://github.com/alejandrobalderas/claude-code-from-source/blob/main/book/ch01-architecture.md) + [`ch05` the agent loop](https://github.com/alejandrobalderas/claude-code-from-source/blob/main/book/ch05-agent-loop.md) — the same loop as actually shipped: async generator, two-layer entry point, six abstractions. Reverse-engineered from npm source maps, so read it as a well-argued claim, not documentation. |
...
| Time | Block | What to do |
|---|---|---|
| 09:00 | **Read A** | • [Writing Effective Tools for Agents](https://www.anthropic.com/engineering/writing-tools-for-agents) — Anthropic<br>• [SWE-agent](https://arxiv.org/abs/2405.15793) — the Agent-Computer Interface section<br>• learn-claude-code [`s02_tool_use`](https://github.com/shareAI-lab/learn-claude-code/tree/main/s02_tool_use)<br>• [Albada, Ch 4: Tool Use](https://www.oreilly.com/library/view/building-applications-with/9781098176495/)<br>• claude-code-from-source [`ch06` tools](https://github.com/alejandrobalderas/claude-code-from-source/blob/main/book/ch06-tools.md) — the 14-step execution pipeline and result budgeting — and [`ch07` concurrency](https://github.com/alejandrobalderas/claude-code-from-source/blob/main/book/ch07-concurrency.md) — the partition algorithm that decides which calls may run in parallel |
...
| Time | Block | What to do |
|---|---|---|
| 09:00 | **Read A** | • learn-claude-code [`s03_permission`](https://github.com/shareAI-lab/learn-claude-code/tree/main/s03_permission) and [`s04_hooks`](https://github.com/shareAI-lab/learn-claude-code/tree/main/s04_hooks)<br>• [The Lethal Trifecta](https://simonwillison.net/2025/Jun/16/the-lethal-trifecta/) — Simon Willison<br>• [OWASP Top 10 for LLM Applications 2025](https://genai.owasp.org/llm-top-10/) — LLM01 (Prompt Injection), LLM06 (Excessive Agency)<br>• [Beyond Permission Prompts](https://www.anthropic.com/engineering/claude-code-sandboxing) and [Claude Code Auto Mode](https://www.anthropic.com/engineering/claude-code-auto-mode)<br>• claude-code-from-source [`ch12` extensibility](https://github.com/alejandrobalderas/claude-code-from-source/blob/main/book/ch12-extensibility.md), the hooks half: lifecycle events, hook sources, and exit-code semantics — how a hook blocks a tool call rather than just observing it |
...
| 14:30 | **Code-read** | • Compare the context boundary in `s06` with [Albada, Ch 5: Orchestration](https://www.oreilly.com/library/view/building-applications-with/9781098176495/).<br>• claude-code-from-source [`ch08` sub-agents](https://github.com/alejandrobalderas/claude-code-from-source/blob/main/book/ch08-sub-agents.md) and [`ch10` coordination and swarms](https://github.com/alejandrobalderas/claude-code-from-source/blob/main/book/ch10-coordination.md) — what a child agent inherits and what it cannot send back; read its *cost of orchestration* section against Cognition's post. |
...
| 15:45 | **Read B** | • [Lost in the Middle](https://arxiv.org/abs/2307.03172) (Liu et al., 2023)<br>• [Context Engineering for Reliable AI Agents: Lessons from Building Azure SRE Agent](https://techcommunity.microsoft.com/blog/appsonazureblog/context-engineering-lessons-from-building-azure-sre-agent/4481200/) — files + `grep` beat 100+ bespoke tools. |
...
| — | **Prep for Monday** | • re-read your notes on [*Hands-On LLMs*, Ch 8](https://www.oreilly.com/library/view/hands-on-large-language/9781098150952/) (semantic search and RAG); download ~50 public MAS notices and guidelines as PDFs from [mas.gov.sg](https://www.mas.gov.sg/regulation). |
...
| — | **Optional evening** | • [Stop Saying RAG Is Dead](https://hamel.dev/notes/llm/rag/not_dead.html) — Hamel Husain<br>• Parser alternatives: [Unstructured](https://github.com/Unstructured-IO/unstructured) · [Azure Document Intelligence](https://learn.microsoft.com/en-us/azure/ai-services/document-intelligence/overview) [Azure · read] · [Textract](https://aws.amazon.com/textract/) [AWS · optional] |
...
| 15:45 | **Read B** | • [*AI Engineering*](https://www.oreilly.com/library/view/ai-engineering/9781098166298/), **Ch 6**, the agents half<br>• [Self-RAG](https://arxiv.org/abs/2310.11511) (Asai et al., 2023), intro |
...
| Time | Block | What to do |
|---|---|---|
| 09:00 | **Read A** | • learn-claude-code [`s09_memory`](https://github.com/shareAI-lab/learn-claude-code/tree/main/s09_memory) and [`s10_task_system`](https://github.com/shareAI-lab/learn-claude-code/tree/main/s10_task_system)<br>• [Effective Harnesses for Long-Running Agents](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents) — Anthropic<br>• [MemGPT](https://arxiv.org/abs/2310.08560) (Packer et al., 2023), §1–3<br>• claude-code-from-source [`ch11` memory](https://github.com/alejandrobalderas/claude-code-from-source/blob/main/book/ch11-memory.md) — a files-and-frontmatter design instead of a vector store: four-type taxonomy, write and recall paths, staleness, `MEMORY.md` as the always-loaded index. A cheap baseline your `MemoryStore` should beat on the temporal query. |
...
| — | **Optional evening** | • [Harness Design for Long-Running Application Development](https://www.anthropic.com/engineering/harness-design-long-running-apps)<br>• [AI Agents for Beginners](https://github.com/microsoft/ai-agents-for-beginners) — the agent memory lesson |
...
| Time | Block | What to do |
|---|---|---|
| 09:00 | **Read A** | • Hamel Husain: [Your AI Product Needs Evals](https://hamel.dev/blog/posts/evals/) · [A Field Guide to Rapidly Improving AI Products](https://hamel.dev/blog/posts/field-guide/) · [LLM Evals FAQ](https://hamel.dev/blog/posts/evals-faq/)<br>• [Demystifying Evals for AI Agents](https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents)<br>• [*AI Engineering*](https://www.oreilly.com/library/view/ai-engineering/9781098166298/) (Chip Huyen), **Ch 3–4** |
...
| 15:45 | **Read B** | • [Judging LLM-as-a-Judge](https://arxiv.org/abs/2306.05685) (Zheng et al., 2023)<br>• [Who Validates the Validators?](https://arxiv.org/abs/2404.12272) (Shankar et al., 2024)<br>• [Quantifying Infrastructure Noise in Agentic Coding Evals](https://www.anthropic.com/engineering/infrastructure-noise) |
| — | **Optional evening** | • [*Evals for AI Engineers*](https://www.oreilly.com/library/view/evals-for-ai/9798341660717/) (Shankar & Husain, O'Reilly)<br>• [Patterns for Building LLM-based Systems & Products](https://eugeneyan.com/writing/llm-patterns/) — Eugene Yan, evals section |
...
| — | **Optional evening — production harnesses** | • [deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) architecture docs · [openai/codex](https://github.com/openai/codex) sandboxing and approval-policy code (Rust)<br>• Field reports (Anthropic *Scaling Managed Agents*; Cognition *What We Learned Building Cloud Agents*; OpenAI *Harness Engineering*) — linked from the [awesome-agent-harness wiki](https://github.com/Picrew/awesome-agent-harness/wiki)<br>• [LangChain Academy](https://academy.langchain.com) — *Introduction to LangGraph*, persistence and HITL modules<br>• claude-code-from-source [`ch18` what we learned](https://github.com/alejandrobalderas/claude-code-from-source/blob/main/book/ch18-epilogue.md) — five architectural bets and *what transfers, what does not*; read it against learn-claude-code's "agents are trained, not coded" stance |
...
| 15:45 | **Read B** | • [Optimizing AI Inference at Character.AI](https://blog.character.ai/optimizing-ai-inference-at-character-ai-2/)<br>• [A Postmortem of Three Recent Issues](https://www.anthropic.com/engineering/a-postmortem-of-three-recent-issues) — Anthropic<br>• [*AI Engineering*](https://www.oreilly.com/library/view/ai-engineering/9781098166298/), **Ch 9** |
| — | **Optional evening** | • [How to Scale Your Model](https://jax-ml.github.io/scaling-book/) — inference chapters · [Stanford CS336](https://cs336.stanford.edu) Lecture 10 · [PagedAttention paper](https://arxiv.org/abs/2309.06180)<br>• claude-code-from-source [`ch17` performance](https://github.com/alejandrobalderas/claude-code-from-source/blob/main/book/ch17-performance.md) — the harness half of unit economics: context-slot reservation, tool-result budgets, prompt-cache design. Serving cost is only one of the two levers. |
...
| 15:45 | **Read B** | • OWASP [Top 10 for Agentic Applications (2026)](https://genai.owasp.org/resource/owasp-top-10-for-agentic-applications-for-2026/) and [Agentic AI — Threats and Mitigations](https://genai.owasp.org/resource/agentic-ai-threats-and-mitigations/) — map every finding to a category |
| — | **Optional evening** | • [*AI Engineering*](https://www.oreilly.com/library/view/ai-engineering/9781098166298/), **Ch 10**<br>• **Day 16 primer:** Birgitta Böckeler, [Understanding Spec-Driven Development: Kiro, spec-kit, and Tessl](https://martinfowler.com/articles/exploring-gen-ai/sdd-3-tools.html) |
...
**Added after the plan was written:** [claude-code-from-source](https://github.com/alejandrobalderas/claude-code-from-source) ([site](https://claude-code-from-source.com)) — 18 chapters on Claude Code's architecture, distilled by AI agents from the TypeScript that shipped inside its npm `.js.map` source maps. **Second reference view, not a second backbone.** learn-claude-code is a teaching rewrite you can run; this is a narrative of one production harness you can only read — no license file, TypeScript, and every code block is the author's pseudocode rather than the original, so treat its specifics as claims to check against your own build. Where it earns its place: `ch07` (parallel tool calls, Day 2), `ch09` (prompt-cache prefix, Day 5), `ch11` (file-based memory, Day 10), `ch15` (MCP implementation choices, Day 9), `ch17` (harness-side token economics, Day 13). Skip `ch13`–`ch14` (terminal UI) and `ch16` (remote execution) — no day depends on them; `ch02`–`ch03` (bootstrap, state) are optional context.
